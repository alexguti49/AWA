...
session_start();
	...
	if($_SESSION['sesion_idtipo']==1) { 
		$sql_Contactos 	= "SELECT 	contactos.idcontacto, 
									contactos.nombre, 
									contactos.apellido, 
									contactos.celular, 
									contactos.estado, 
									...
									grupos.grupo,
									grupos.idgrupo 
							FROM 	contactos, usuarios, grupos
							WHERE 	contactos.idgrupo = grupos.idgrupo 
							AND 	usuarios.idusuario = grupos.idusuario";
		$rs_Contactos 	= $mysqli->query($sql_Contactos);
		$rows_Contactos = $rs_Contactos->num_rows;
	} else {
		$sql_Contactos 	= "SELECT 	contactos.idcontacto, 
									contactos.nombre, 
									contactos.apellido, 
            ...
            <div class="panel panel-primary">
            	<div class="panel-heading">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-primary btn-block" style="width:100px;" onClick="location.href='contactos_registrar'">NUEVO
                        </button>	
                    </div>
               	</div>
              	<div class="panel-body">
                
                <?php if($rows_Contactos > 0) { ?>
                ...
                <table align="center" class="display" id="example" style="padding:10px 0px;">
                    ...
                    <tbody>
                        <?php while($row_Contactos = $rs_Contactos->fetch_assoc()){ ?>
							<?php 
								if ($row_Contactos['estado'] == 1) 
								{
									$estado = "Activo";
								} 
								else 
                            ...
                            ?>
                        <tr>
                            <td><?php echo $nombre = encrypt_decrypt('decrypt', $row_Contactos['nombre']);?></td>
                            <td><?php echo $apellido = encrypt_decrypt('decrypt', $row_Contactos['apellido']);?></td>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Contactos['celular']);?></td>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Contactos['grupo']);?></td>
                            <?php if($_SESSION['sesion_idtipo']==1) { ?>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Contactos['usuario']);?></td>
                            <?php } ?>
...
<td align="center">
    <div class="div-editar">
        <a class="fa-editar fa fa-pencil-square-o" aria-hidden="true" title="Editar" href="contactos_modificar?_id=<?php echo encrypt_decrypt('encrypt', $row_Contactos['idcontacto']);?>"></a>
    </div>

    <div class="div-eliminar">
    	<a class="fa-eliminar fa fa-trash" aria-hidden="true" title="Eliminar" href="contactos_eliminar?_id=<?php echo encrypt_decrypt('encrypt',$row_Contactos['idcontacto']);?>" onclick="return confirm('¿Está seguro de eliminar el Contacto: <?php echo $nombre." ".$apellido ?>? ¡Si acepta perdera toda su informacion!');"></a>
    </div>
</td>
						</tr>
						<?php } ?>
