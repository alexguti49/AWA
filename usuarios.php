...
session_start();
	...
	$sql_Usuarios = "SELECT usuarios.idusuario,
							usuarios.usuario,
							usuarios.nombre,
							usuarios.apellido,
							usuarios.email,
							usuarios.idtipo,
							usuarios.estado,
							usuarios_tipos.idtipo, 
							usuarios_tipos.tipo 
					FROM 	usuarios, usuarios_tipos
					WHERE 	usuarios_tipos.idtipo = usuarios.idtipo";
	$rs_Usuarios = $mysqli->query($sql_Usuarios);
	$rows_Usuarios = $rs_Usuarios->num_rows;
            ...
            <div class="panel panel-primary">
            	<div class="panel-heading">
                    ...
                    <div class="input-group">
                        ...
                        <?php if($_SESSION['sesion_idtipo']==1) { ?>
                        <button type="button" class="btn btn-primary btn-block" style="width:100px;" onClick="location.href='usuarios_registrar'">NUEVO
                        </button><?php } ?>	
                    </div>
                </div>
                ...
                <table align="center" class="display" id="example" style="padding:10px 0px;">
                    ...
                    <tbody>
                        <?php while($row_Usuarios = $rs_Usuarios->fetch_assoc()){ ?>
						<?php 
                            if ($row_Usuarios['estado'] == 1) { $estado = "Activo"; } 
                            else { $estado = "Inactivo"; }
                        ?>
                        
                        <tr>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Usuarios['nombre']);?></td>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Usuarios['apellido']);?></td>
                            <td><?php echo $usuario = encrypt_decrypt('decrypt', $row_Usuarios['usuario']);?></td>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Usuarios['email']);?></td>
                            <td><?php echo encrypt_decrypt('decrypt', $row_Usuarios['tipo']);?></td>
                            <?php if($_SESSION['sesion_idtipo']==1) { ?>
                            <td><?php echo $estado;?></td>

<td align="center">
    <div class="div-editar">
        <a class="fa-editar fa fa-pencil-square-o" aria-hidden="true" title="Editar" href="usuarios_modificar?_id=<?php echo encrypt_decrypt('encrypt', $row_Usuarios['idusuario']);?>"></a>
    </div>

    <div class="div-eliminar">
    	<a class="fa-eliminar fa fa-trash" aria-hidden="true" title="Eliminar" href="usuarios_eliminar?_id=<?php echo encrypt_decrypt('encrypt',$row_Usuarios['idusuario']);?>" onclick="return confirm('¿Está seguro de eliminar el Usuario: <?php echo $usuario ?>? ¡Si acepta perdera toda su informacion!');"></a>
    </div>
</td>
<?php } ?>
						</tr>
