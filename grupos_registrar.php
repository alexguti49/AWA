...
session_start();
	...
	$sql_idusuario = "SELECT * FROM usuarios WHERE idusuario != '$sesion_idusuario'";
	...
	if(!empty($_POST))
	{
		...
		$descripcion_cifrado= encrypt_decrypt('encrypt', $descripcion);
			...
			$sql_Grupo_val 	= "SELECT * FROM grupos WHERE grupo ='$grupo_cifrado' AND idusuario = '$sesion_idusuario'";
			...
			if($rows_Grupo_val > 0) { 
				...
				$mensaje = "Error al registrar informacion del grupo. El grupo: $grupo ya existe para el usuario: $usuario.";
				echo "<script>";
				echo "alert('$mensaje');";
				echo "history.back();";
				echo "</script>";
			} else {
				$sql_Grupo_add = "INSERT INTO grupos (grupo, descripcion, estado, idusuario)
								  VALUES('$grupo_cifrado', '$descripcion_cifrado', '$estado', '$idusuario')";
				$rs_Grupo_add = $mysqli->query($sql_Grupo_add);
				
				$grupo = encrypt_decrypt('decrypt', $grupo_cifrado);
				
				if($rs_Grupo_add > 0) {
					$mensaje = "Informacion del grupo: $grupo registrada correctamente.";
					echo "<script>";
					echo "alert('$mensaje');";
					echo "window.location = 'grupos';";
					echo "</script>";
				} else {
					$mensaje = "Error al registrar informacion del grupo: $grupo.";
					echo "<script>";
					echo "alert('$mensaje');";
					echo "history.back();";
					echo "</script>";
    ...
    $("#_grupo").keyup(function(e)
	{
        ...
        //hace la búsqueda
		 ...
		 {      
			...
			({
			  	type: "POST",
			  	url: "comprobar_grupo_add.php",
			  	data: "grupo="+grupo.toUpperCase(),
			  	dataType: "html",
			  	error: function()
...
//*******************************************************************************************************
function validarGruposRegistrar() //VALIDA LOS CAMPOS DEL FORMULARIO
{	
	/*if( validarGrupo() && validarDescripcion() && validarEstado() ) 
		{	
		...
		<div class="col-sm-offset-2 col-sm-8   col-md-offset-3 col-md-6    col-lg-offset-3 col-lg-6    col-xs-12">
        	<div class="panel panel-primary"> 
                ...
                <div class="panel-heading">
					<h3 align="center">REGISTRAR GRUPO</h3>
				</div>
      	    	...
      	    	<form id="RegistrarDatos" name="RegistrarDatos" class="form-horizontal" action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
              		...
              		<div class="form-group">
				        <div class="input-group">
					        ...
					        <input type="text" class="form-control" id="_grupo" name="_grupo" maxlength="20" style="text-transform:uppercase">
					  	</div>
			     	</div>
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            <div class="div-titulos">
                                <label class="span-titulos control-label">Descripcion:</label>
                            </div> 
                            <span class="input-group-addon"><i class="fa fa-users"></i></span>
                            <textarea class="form-control" id="_descripcion" name="_descripcion" type="text" maxlength="100"></textarea>
                        </div>
                    </div>
                        ...
                        <div class="input-group">
                            ...
                            <span class="input-group-addon"><i class="fa fa-users"></i></span>
                                <select class="form-control" id="_idusuario" name="_idusuario">
                                    <option value="<?php echo $row_user['idusuario'];?>"><?php echo encrypt_decrypt('decrypt', $row_user['usuario']);?></option>
                   ...
                   <!------------------LINEA CELESTE-------------->
                    ...
                    <div class="form-group">
                        <div align="center" class="panel-negro">
                            <input type="submit" class="btn btn-primary" value="Registrar" onClick="validarGruposRegistrar();">
                        </div>
                    </div>
